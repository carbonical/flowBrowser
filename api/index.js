    ...
    $('a, img, video, form, link[rel="stylesheet"], script[src], link[rel="icon"], link[rel="apple-touch-icon"]').each((i, el) => {
      ...
      if (tagName === 'a') {
        ...
        if (href && !href.startsWith('http')) {
          $(el).attr('href', `/api/index.js?url=${targetUrl}${encodeURIComponent(href)}`);
        }
      } else if (tagName === 'img') {
        src = $(el).attr('src');
        if (src && !src.startsWith('http')) {
          $(el).attr('src', `/api/index.js?url=${targetUrl}${encodeURIComponent(src)}`);
        }
      } else if (tagName === 'video') {
        const poster = $(el).attr('poster');
        if (poster && !poster.startsWith('http')) {
          $(el).attr('poster', `/api/index.js?url=${targetUrl}${encodeURIComponent(poster)}`);
        }
        src = $(el).attr('src');
        if (src && !src.startsWith('http')) {
          $(el).attr('src', `/api/index.js?url=${targetUrl}${encodeURIComponent(src)}`);
        ...
        if (action && !action.startsWith('http')) {
          $(el).attr('action', `/api/index.js?url=${targetUrl}${encodeURIComponent(action)}`);
        }
